...
kubectl apply -f k8s
kubectl set image deployments/server-deployment server=masbot/multi-server:$SHA
kubectl set image deployments/client-deployment client=masbot/multi-client:$SHA
kubectl set image deployments/worker-deployment worker=masbot/multi-worker:$SHA